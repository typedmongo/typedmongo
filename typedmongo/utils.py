...
def snake_case(name: str) -> str:
    ...
    """
    return "".join("_" + char.lower() if char.isupper() and i > 0 else char.lower() for i, char in enumerate(name))


class OnlyUseAsClass(type):
    def __call__(self, *args, **kwds) -> NoReturn:
        raise NotImplementedError(f"The class {self.__name__} cannot be instantiated, please use it directly.")


def safe_issubclass(obj: Any, class_or_tuple: Union[type, Tuple[Union[type, Tuple[Any, ...]], ...]]) -> bool:
